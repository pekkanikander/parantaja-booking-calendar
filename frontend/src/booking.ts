...
import { postBooking, ConflictError } from "./api";
...
export function showBookingForm(
  ...
  const endLabel = slotEnd.toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" });
  ...
  document.getElementById("bf-cancel")!.onclick = () => { el.style.display = "none"; };
  ...
  document.getElementById("bf-submit")!.onclick = async () => {
    ...
    try {
      ...
      errorEl.textContent = err instanceof ConflictError
    ...
    calendar.getEventById(slot.start)?.remove();
...
function showConfirmation(name: string, startLabel: string, endLabel: string): void {
  ...
  `;
  (document.getElementById("conf-msg") as HTMLElement).textContent =
    `Thanks, ${name}! Your appointment at ${startLabel} – ${endLabel} is booked.`;
  document.getElementById("bf-close")!.onclick = () => { el.style.display = "none"; };
  el.style.display = "block";
}
